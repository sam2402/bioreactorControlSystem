...
class BioreactorApp():
    ...
    def read(self):
        ...
        #values = {"Temperature (°C)": arduinoData[0], "Stirrer RPM": arduinoData[1], "pH": arduinoData[2]}
        values = {"Temperature (°C)": random.uniform(
            25, 35), "Stirrer RPM": random.uniform(500, 1500), "pH": random.uniform(3, 7)}
        return values
...
class SetPointsPage(Frame):
    ...
    def write(self):
        ...
        if(values["Temperature (°C)"] != self.oldHeating or values["Stirrer RPM"] != self.oldStirring or values["pH"] != self.oldPh):
            #arduino.write(bytes(str(heating)+" "+str(stirring)+" "+str(ph), 'utf-8'))
            self.oldHeating = values["Temperature (°C)"]
            self.oldStirring = values["Stirrer RPM"]
            self.oldPh = values["pH"]
        self.controller.root.after(200, self.write)

    def updateVal(self, label, parameter):
        values = self.controller.read()
        label["text"] = "Current pH value: " + str(values[parameter])
